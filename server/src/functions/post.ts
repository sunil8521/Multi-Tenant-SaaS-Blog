...
export const GetAllPost = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!team) return next(new ErrorHandler(404, "Team not found"));
    const filters: any = {
      published: true,
      teamId: team.id,
    };
...
export const GetPost = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    delete (response as any).bookmarks;
...
export const GetPostComments = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
...
export const GetReplyComments = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!postId || !commentId) {
      return next(
        new ErrorHandler(400, "Post ID and Comment ID are required.")
      );
    }
...
export const CreatePostComment = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    const { postId } = req.params;
    ...
    if (!postId || !authorId) {
      return next(
        new ErrorHandler(
          400,
          "Unable to add comment — missing post or user information."
        )
      );
    }
...
export const CreatePostReply = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    const { postId, commentId } = req.params;
    const { content, authorId } = req.body;
    ...
    if (!postId || !authorId) {
      return next(
        new ErrorHandler(
          400,
          "Unable to add reply — missing post or user information."
        )
      );
    }
