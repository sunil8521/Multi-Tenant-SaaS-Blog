        ...
        before: createAuthMiddleware(async (ctx) => {
            ...
            if (!origin) {
                throw new APIError("BAD_REQUEST", {
                    message: "Missing origin header. Please try signing in from the application or contact support if the problem persists.",
                });
            }
            ...
            if (!subdomain) {
                throw new APIError("BAD_REQUEST", {
                    message: "Missing subdomain. Please try signing in from the application or contact support if the problem persists.",
                });
            }
            // Check if user exists and is member of the team for this subdomain
            const userTeamMembership = await checkUserTeamMembership(email, subdomain);
